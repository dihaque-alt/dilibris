...
import { addNotification, loadNotifSeen, saveNotifSeen } from './notificationsStore';

/** Pull recent buddy/challenge activity into the local notification feed. */
export async function syncActivityNotifications(userId: string): Promise<void> {
  const seen = loadNotifSeen(userId);
  ...
  for (const msg of messages ?? []) {
    ...
    seen[key] = msg.created_at as string;
  ...
  for (const note of notes ?? []) {
    ...
    if (seen[key]) continue;
    seen[key] = note.created_at as string;
  ...
  if (challenge?.target_books) {
    ...
    if (finished >= Math.ceil(target / 2) && finished < target && !seen[halfKey]) {
      seen[halfKey] = now.toISOString();
      addNotification(userId, {
        id: halfKey,
        kind: 'challenge',
